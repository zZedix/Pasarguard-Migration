...
if [[ -d "$MARZBAN_DIR" ]]; then
  ...
  if [[ -f "$MARZBAN_DIR/.env" ]]; then
    log "Moving .env to new directory"
    mv "$MARZBAN_DIR/.env" "$PASARGUARD_DIR/.env"
  fi
fi
...
if [[ -d "$MARZBAN_DATA" ]]; then
  ...
  if [[ -d "$MARZBAN_DATA/certs" ]]; then
    log "Moving certs directory"
    mkdir -p "$PASARGUARD_DATA"
    mv "$MARZBAN_DATA/certs" "$PASARGUARD_DATA/certs"
  fi
else
